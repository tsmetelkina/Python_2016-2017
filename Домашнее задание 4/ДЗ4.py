...
def _insert_ ():
    ...
    for token in lines_token:
        ...
        for token_cl in d:
            if t == token_cl:
                string = 'INSERT INTO The_First_Table (ID, Token, Punkt_mark_left, Punkt_mark_right, Token_num, Token_ID) VALUES ("%d", "%s", "%s", "%s", "%d", "%d");\n' %(i, token.strip (',.-?!;:()«»"'), punkt_left, punkt_right, i, d[token_cl])
            else:
                continue
        f3 = open ('insert2.txt', 'a', encoding = 'utf-8')
        f3.write (string)
